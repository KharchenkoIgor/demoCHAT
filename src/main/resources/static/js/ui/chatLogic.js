import { fetchServers, fetchChannels, fetchHistory, fetchMyRole,
    deleteMessage, deleteChannel, deleteServer, updateMessage } from '../api/api.js';
...
export async function initChatPage() {
    try {
        ...
        Array.from(serverList.children).forEach(c => !['add-server-button', 'home-button'].includes(c.id) && c.remove());
